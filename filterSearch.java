...
public class filterSearch {
	...
	String image1Path = "\"G:\\The Programmer's Delight\\Eclipse Projects\\Photos For MintLeaf\\filter.png\""; // download from the images branch in github and change the path
	Connection con;
	...
	public filterSearch() {
		...
		filterButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				// if no checkbox is selected, close the frame and return to homepage
				if (!selected) {filterFrame.dispose();} 
				else {
				...
				if (component instanceof JCheckBox) { // if there is a checkbox in a panel
					...
					if (checkBox.isSelected()) { // and it is selected
						...
						if (!isOnlyFilter) { // if it is not the only selected checkbox
							// and if it is the only checkbox in the panel, 
							// search for titles that match both cateogries (ex. Rating is 4 and Language is Spanish)
							if (isOnlyCheckboxInPanel) { queryBuilder.append(" AND");} 
							
							// otherwise, if there is another checkbox in the same panel, 
							// search for titles that match either cateogry (ex. Genre is Fantasy or Historical)
							else {queryBuilder.append(" OR");}	
						} // end if
		
						// if it is the only selected checkbox, move on to next selected checkbox
						else { isOnlyFilter = false;} // end else
						...
						for (Component labelComponent : panelComponents) { // for each label in the panel
							if (labelComponent instanceof JLabel) {
								...
								System.out.println(label.getText().replace(':', ' ').trim() + ": " + checkBox.getText());
							}
						} // end for loop
	...
	private void searchDatabase(String query) {
		try (Connection con = DriverManager.getConnection(Tales.conPath, Tales.dbUsername, Tales.dbPassword)) {
			...
			try (ResultSet res = prep.executeQuery()) {
				...
				// otherwise, print out matching titles
				while (res.next()) {
			...
			System.err.println("ERROR - CONNECTION UNSUCCESSFUL; Searching database with filters");
		} // end try/catch
	} // end searchDatabase
